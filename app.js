...
(function () {
  ...
  function initRadarSwitcher() {
    ...
    function setView(key) {
      ...
      cards.forEach(function (card) {
        var isActive = normalizedKey !== 'overview' && card.getAttribute('data-radar-view') === normalizedKey;
        card.classList.toggle('is-active', isActive);
        card.setAttribute('aria-pressed', isActive ? 'true' : 'false');
      });
  ...
  function initRadarCards() {
    var wrap = qs('[data-radar-switcher]');
    var cards = qsa('[data-radar-view]');
    var scoreCard = qs('.composite-score-card');
    if (!wrap || !cards.length || typeof wrap._setRadarView !== 'function') return;

    cards.forEach(function (card) {
      card.addEventListener('click', function () {
        wrap._setRadarView(card.getAttribute('data-radar-view'), { scroll: true, focus: true });
      });
    ...
    if (scoreCard) {
      ...
      scoreCard.setAttribute('aria-label', '切换到综合四维总览');
      scoreCard.addEventListener('click', function () {
        wrap._setRadarView('overview', { scroll: true, focus: true });
      });
